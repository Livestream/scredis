...
class RedisConfig(config: Config = ConfigFactory.load().getConfig("scredis")) {
  ...
  private def parseFiniteDuration(key: String, value: String): FiniteDuration = {
    ...
    if(duration.isFinite) new FiniteDuration(duration.length, duration.unit)
    else throw new IllegalArgumentException("%s must be finite".format(key))
  }
  ...
  object Pool {
    ...
    Config.maxWait = if(MaxWait.isFinite) {
      new FiniteDuration(MaxWait.length, MaxWait.unit).toMillis
    } else {
      -1
    }
